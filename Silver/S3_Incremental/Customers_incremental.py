...
BRONZE_FILE = "Customers.csv"

SILVER_URI = "s3://sales-mini/silver/Customers/Customers.csv"
PK_CANDIDATES = ["customer_id", "customerid"]
...
def main():
    ...
    if not pk:
        raise ValueError(f"Customers: missing PK. Found columns: {list(incoming.columns)}")
    ...
    print(f"✅ Customers upserted -> {SILVER_URI} | rows={len(merged):,}")
