...
BRONZE_FILE = "Orders.csv"

SILVER_URI = "s3://sales-mini/silver/Orders/Orders.csv"
PK_CANDIDATES =  ["order_id", "orderid"]
UPDATED_CANDIDATES = ["updatedat", "updated_at", "ingestion_ts", "orderdateutc"]
...
def main():
    ...
    if not pk:
        raise ValueError(f"Orders: missing PK. Found columns: {list(incoming.columns)}")
    ...
    print(f"✅ Orders upserted -> {SILVER_URI} | rows={len(merged):,}")
