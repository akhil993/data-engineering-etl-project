...
BRONZE_FILE = "Products.csv"
SILVER_URI = "s3://sales-mini/silver/Products/Products.csv"
PK_CANDIDATES = ["product_id", "productid", "sku"]
...
def main():
    ...
    if not pk:
        raise ValueError(f"Products: missing PK. Found columns: {list(incoming.columns)}")
    ...
    print(f"✅ Products upserted -> {SILVER_URI} | rows={len(merged):,}")
